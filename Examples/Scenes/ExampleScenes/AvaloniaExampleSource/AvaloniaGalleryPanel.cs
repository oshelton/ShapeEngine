...
/// <c>ScrollViewer</c>s, text input, a radio group, a checkbox and a toggle switch all share the surface
/// with five kinds of ShapeEngine icon, plus a static, an animated and a direct view used purely as
/// images - all at once, each hit-testable or not exactly as its plain Avalonia counterpart would be.
/// The Icon buttons tab carries that further: the same five icons appear as the content of a
/// <c>Button</c>, a <c>RadioButton</c>, a <c>ToggleButton</c> and a <c>CheckBox</c>, each control keeping
/// its own selection model over the same underlying render.
/// The animated and direct views on the ShapeEngine tab carry the more elaborate drawing: concentric orbit
/// rings and a bar chart, rather than the simpler per-shape icons everything else on the tab uses. The
/// static emblem beneath them is seeded and regenerated on its own button, independent of the hero's.
/// The Animation tab is a different kind of motion entirely - keyframe animations, property transitions
/// and cross-fades, all Avalonia's own, driven by the same render tick that draws every other tab.
/// </remarks>
public sealed class AvaloniaGalleryPanel : ViewBase
{
    ...
    /// <summary>Selects a tab by index (0 = Controls, 1 = Icon buttons, 2 = ShapeEngine, 3 = Animation). TEMP verification.</summary>
    ...
    /// rather than an icon font or bitmap asset: a <c>Button</c> that selects one, a <c>RadioButton</c>
    /// group that picks exactly one, a <c>ToggleButton</c> row of independent favorites, and a
    /// <c>CheckBox</c> row of independent inclusion flags.
    /// </summary>
    /// <remarks>
